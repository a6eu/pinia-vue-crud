  ...
  <div class="flex flex-col">
    <div class="flex justify-between items-center my-9 w-[500px]">
      ...
      <button @click="fetchProducts">
        <img
            class="w-10"
            src="../assets/Refresh%20Cw%20Alt%203%20Icon.svg"
            alt=""
        />
      </button>
    </div>
    <div class="flex gap-5 items-start">
      <div class="w-[500px]">
        <ul class="rounded-2xl border overflow-hidden bg-white">
          <li
              v-if="!loading"
              class="even:bg-gray-100 h-10 px-2.5 flex items-center justify-between"
          ...
          >
            ...
            <div>
              <button
                  class="bg-red-600 hover:opacity-80 rounded text-white px-2 mr-2"
                  @click="removeProduct(product.id)"
              >
                Delete
              </button>
              <button class="underline" @click="populateProduct(product)">
                Update
              </button>
            </div>
          </li>
          <li v-else class="text-center py-3">Loading...</li>
        </ul>
        <button
            v-if="!loading && products.length > 0"
            @click="loadMore"
            class="w-full mt-4 bg-black text-white px-4 py-2 rounded-2xl"
        >
          Load More
        </button>
      </div>
      ...
      <div class="bg-white border rounded-2xl py-3 px-6">
        ...
        <form
            ...
            @submit.prevent="editMode ? updateProduct() : addNewProduct()"
        >
          <input
          ...
          />
          <button
              class="bg-black hover:opacity-80 items-center rounded text-white px-2"
              type="submit"
          >
            {{ editMode ? "Update" : "+" }}
          </button>
